...
int main(void)
{
    ...
    try {
        ...
        struct Employee emp = { 0 };    // Initialize an Employee struct, emp, to a safe empty state

        do {
            // Call the menu() and proceed to different case based on user selection
            selection = menu();
            switch (selection) {
            case 1:
...
int menu(void) {
    ...
    do {
        ...
        cin >> option;
    } while (option < 0 || option > 5);     // Make sure the user is entering a number between 0 and 5
    return option;
}
...
int findEmployee(Connection* conn, int employeeNumber, struct Employee* emp) {
    ...
    try {
        ...
        stmt->setNumber(1, employeeNumber);     // get the employee number from the user input
...
void displayAllEmployees(Connection* conn)
{
    ...
    try {
        ...
        if (!rs->next()) {	// if the query does not return any rows
            ...
            do {
                ...
                // using width() from iostream library to set the filed width for a formatted output stream
                cout.width(10);
...
void insertEmployee(struct Employee* emp) {
    ...
    cin >> emp->email;
    cout << "Office Code : 1\n";
    strcpy(emp->officecode, "1");   // assign the office code(1) and display it on the screen
    cout << "Manager ID : 1002\n";
    emp->reportsTo = 1002;          // assign the manager id(1002) and display it on the screen
    cin.ignore();                   // clear the buffer so that user can enter jobtitle from the next cin function
    cout << "Job Title : ";
    cin >> emp->jobTitle;
}
...
void insertEmployee(Connection* conn, struct Employee emp) {
    ...
    if (returned != 0) {
        ...
        try {
            ...
            conn->commit();     // Commit insert statement to make the change permanent
            conn->terminateStatement(stmt);
        }
        catch (SQLException& sqlExcp) {
            cout << sqlExcp.getErrorCode() << ": " << sqlExcp.getMessage();
...
void updateEmployee(Connection* conn, int employeeNumber) {
    ...
    if (returned == 0) {
        ...
        try {
            ...
            conn->commit();     // Commit update statement to make the change permanent
            conn->terminateStatement(stmt);
        }
        catch (SQLException& sqlExcp) {
            cout << sqlExcp.getErrorCode() << ": " << sqlExcp.getMessage();
...
void deleteEmployee(Connection* conn, int employeeNumber) {
    ...
    if (returned == 0) {
        ...
        try {
            ...
            conn->commit();     // Commit delete statement to make the change permanent
            conn->terminateStatement(stmt);
        }
        catch (SQLException& sqlExcp) {
            cout << sqlExcp.getErrorCode() << ": " << sqlExcp.getMessage();

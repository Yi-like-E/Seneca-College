...
function updateCardTitle(title) {
  let id = document.querySelector("#card-title");
  let span = id.querySelector("span");
...
//   let cell = createTableCell(child);
...
//   let date = formatted.replace(/(\d{1,2})\/(\d{1,2})\/(\d{4})/, "$3-$1-$2");
//   time.dateTime = date;
//   time.appendChild(createText(date));
//   return time;
// }
...
function cardBody(name, date, uri, wikipediaUrl){
  ...
  let anchor = createAnchor(wikipediaUrl, name);
  h3.appendChild(anchor);
  div.appendChild(h3);
  let anchor2 = createAnchor(uri, formatted);
  h4.appendChild(anchor2);
  div.appendChild(h4);
  return div;
}
...
function buildCardForObservation(observation){
  ...
  let img = cardImg(observation.photoUrl);
  div.appendChild(img);
  let body = cardBody(observation.name, observation.date, observation.uri, observation.wikipediaUrl);
  div.append(body);
  let icon = cardIcons(observation.isNative, observation.isIntroduced, observation.isThreatened, observation.isEndangered);
  div.appendChild(icon);
  let observationData = document.querySelector("#observation-cards");
  observationData.appendChild(div);
  return div;
}
